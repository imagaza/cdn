...
repleteModal.close = rplm.close = function (params) {
  ...
  if(typeof(params.modalNOverlay) !== 'undefined' && params.modalNOverlay == "custom"){
...
repleteModal.fixPosition = rplm.fixPosition = function (errorMessage,animateError) {
  var modal = _repleteModalInitialize$getModal$getOverlay$getInput$setFocusStyle$openModal$resetInput$fixVerticalPosition.getModal();
  ...
  if (typeof getComputedStyle !== 'undefined') {
    // IE 8
    padding = parseInt(getComputedStyle(modal).getPropertyValue('padding-top'), 10);
  } else {
    padding = parseInt(modal.currentStyle.padding);
  }

  modal.style.left = '';
  // modal.style.display = 'none';
  modal.style.marginTop = '-' + parseInt((height + padding) / 2) + 'px';
}
repleteModal.showInputError = rplm.showInputError = function (errorMessage,animateError) {
  animateError = animateError || false;
  var modal = _repleteModalInitialize$getModal$getOverlay$getInput$setFocusStyle$openModal$resetInput$fixVerticalPosition.getModal();
  ...
  if(animateError){
    $(modal).removeClass('animated wobble');
    setTimeout(function(){
      $(modal).addClass('animated wobble')
    },100);
  }
};
...
var handleButton = function handleButton(event, params, modal) {
  ...
  switch (e.type) {
    ...
    case 'click':
      ...
      } else if (_hasClass$isDescendant.isDescendant(modal, target) && target.tagName === 'BUTTON' && target.className === 'cancel') {
...
var setParameters = function setParameters(params) {
  ...
  if(params.type === "input"){
    $fieldset.innerHTML = "<fieldset>\n <input type=\"text\" />\n <div class=\"rm-input-error\" style=\"display:none\"></div>\n </fieldset>";
  }
  else {
    $fieldset.innerHTML = "";
  }
